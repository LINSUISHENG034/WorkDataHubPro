...
**Objective:** Create the shared Phase 02.1 runtime foundation before touching the individual replay slices.
...
| Threat | Mitigation |
|--------|------------|
| T-02.1-01 (reference-derivation baseline loading) | `load_required_checkpoint_baseline()` raises immediately when baseline absent |
| T-02.1-02 (duplicate-row diff reporting) | Fixed `_build_diff()` to use true multiset subtraction |
| T-02.1-03 (baseline bootstrap provenance) | Explicit bootstrap script, no automatic replay fallback |
