...
_DOMAIN = "annual_award"
_PUBLICATION_POLICY_PATH = Path("config/policies/publication.json")
_RELEASE_PATH = Path("config/releases/2026-04-11-annual-award-baseline.json")
_DOMAIN_CONFIG_PATH = Path("config/domains/annual_award/cleansing.json")
...
def _stable_fact_id_from_fields(fields: dict[str, object]) -> str:
    sheet_token = {
        "TrusteeAwards": "trustee",
        "InvesteeAwards": "investee",
    }.get(str(fields.get("source_sheet")), "row")
    return f"award-{sheet_token}-{int(fields['source_row_no']):03d}"
...
def run_annual_award_slice(
    *,
    ...
    execution = execute_replay_run(
        ...
        domain=_DOMAIN,
        intake_service=AnnualAwardIntakeService(),
    ...
    storage = InMemoryTableStore(
        seed={
            ...
            "fixture_annual_loss": _load_replay_rows(
