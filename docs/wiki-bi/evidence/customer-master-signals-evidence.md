...
| E-CMS-007 | legacy_doc | strong | absorbed | `customer-master-signals-evidence`, `primary-branch`, `key-annuity-plan`, `related-branch-count`, `tags` | 2026-04-18 | `E:\Projects\WorkDataHub\docs\verification_guide_real_data.md` 第 5 步把 `主拓机构`、`关键年金计划`、`关联计划数`、`关联机构数`、`其他年金计划`、`其他开拓机构`、`tags` 与 `年金客户类型` 视为 operator 需要显式核验的结果，而不是可忽略的实现副产物。 |
...
- `其他年金计划`、`其他开拓机构` 等字段是 relationship-breadth list signals；它们用来补充 breadth context，但不替代 dominant value 或 breadth count。
...
- `关键年金计划`、`关联计划数` 与 `关联机构数` 已提升为独立 object page；`其他年金计划`、`其他开拓机构` 仍继续由本页承担 cross-domain dispatcher。
