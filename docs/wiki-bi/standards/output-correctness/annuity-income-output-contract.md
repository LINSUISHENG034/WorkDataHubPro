...
其中 `mapping."组合计划"` 的稳定锚点是 [组合代码](../../concepts/portfolio-code.md)；它回答 portfolio 语境，不回答客户身份。
...
- [`其他年金计划`](../../concepts/other-annuity-plans.md) 现在也有独立对象页；它回答计划侧 breadth-list context，不等于 fee-weighted 主导计划，也不等于 breadth count
- [`其他开拓机构`](../../concepts/other-branches.md) 现在也有独立对象页；它回答机构侧 breadth-list context，不等于主导机构选择，也不等于 breadth count
