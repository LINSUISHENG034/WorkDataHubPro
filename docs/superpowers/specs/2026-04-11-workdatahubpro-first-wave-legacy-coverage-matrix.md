...
| CT-005 | history-aware event-domain contract lookup and temporal enrichment semantics | mechanism | `docs/domains/annual_award-capability-map.md`, `docs/domains/annual_loss-capability-map.md`, legacy contract lookup behavior | future temporal lookup contract for event-domain plan-code enrichment | `capabilities` + `reference` | merged first-wave legacy coverage risk register + future annual loss / event-domain parity follow-on plan | `pending` | future lookup contract tests, historical replay evidence, and event-domain slice validation | N/A | current accepted event-domain slices prove replay-backed and current-row lookup behavior |
