...
| Row ID | Behavior / Asset | Category | Legacy Source | Rebuild Target | Target Boundary | Owning Spec / Plan | Status | Validation Evidence | Retirement Decision | Notes / Risks |
|------|-------------------|----------|---------------|----------------|-----------------|--------------------|--------|---------------------|--------------------|---------------|
| AI-001 | single-sheet workbook intake | capability | legacy domain migration workflow and current blueprint archetype mapping | `capabilities/source_intake/annuity_income/service.py` | `capabilities` | `2026-04-14-workdatahubpro-annuity-income-validation-slice.md` | `accepted` | `tests/integration/test_annuity_income_intake.py`, `tests/replay/test_annuity_income_slice.py` | N/A | accepted in validation mode with stable `收入明细` intake contract and anchor-preserving row capture |
| AI-002 | canonical cleansing and fact processing | capability | legacy domain workflow and cleansing-rule references | `capabilities/fact_processing/annuity_income/`, `config/domains/annuity_income/cleansing.json`, `config/releases/2026-04-14-annuity-income-baseline.json` | `capabilities` + `config` | `2026-04-14-workdatahubpro-annuity-income-validation-slice.md` | `accepted` | `tests/integration/test_annuity_income_processing.py`, `tests/replay/test_annuity_income_slice.py` | N/A | accepted in validation mode with explicit branch mapping overrides and fixed-fee defaults |
| AI-003 | identity/reference/publication path | mechanism | legacy enrichment/backfill/load expectations | shared identity contract plus annuity-income derivation/publication wiring | `capabilities` + `platform` | `2026-04-14-workdatahubpro-annuity-income-validation-slice.md` | `accepted` | `tests/integration/test_annuity_income_operator_artifacts.py`, `tests/integration/test_reference_derivation.py`, `tests/integration/test_publication_service.py`, `tests/replay/test_annuity_income_slice.py` | N/A | accepted in validation mode with explicit publication boundaries and temp-id governance; broader cross-domain identity/runtime breadth remains outside this row |
| AI-004 | unresolved-name and failed-record operator artifacts | operator | `docs/domains/annuity_income-capability-map.md`, legacy service export behavior | annuity-income artifact exporter, replay baseline, and runbook coverage | `capabilities` + `apps` + `docs` | `2026-04-14-workdatahubpro-annuity-income-validation-slice.md` | `accepted` | `tests/integration/test_annuity_income_operator_artifacts.py`, `tests/contracts/test_annuity_income_replay_assets.py`, `tests/replay/test_annuity_income_slice.py`, `tests/replay/test_annuity_income_explainability_slo.py` | N/A | income-specific artifact visibility is accepted in validation mode; broader cross-domain artifact parity remains tracked under `CT-016` |
| AI-005 | service-delegation execution path and explicit no-hook contract | mechanism | `docs/domains/annuity_income-capability-map.md`, legacy service path and hook-registry absence | `apps/orchestration/replay/annuity_income_slice.py`, `apps/etl_cli/main.py`, no-projection publication contract | `capabilities` + `apps` | `2026-04-14-workdatahubpro-annuity-income-validation-slice.md` | `accepted` | `tests/integration/test_publication_service.py`, `tests/contracts/test_replay_cli_contracts.py`, `tests/contracts/test_replay_run_report.py`, `tests/replay/test_annuity_income_slice.py` | N/A | accepted in validation mode with replay and CLI entrypoints while remaining outside the projection/hook path |
...
| CT-017 | semantic-map semantic-first canonical alignment and wiki-absorption support | governance | `E:\Projects\WorkDataHub\docs\business-background\客户主数据回填与状态来源分析.md`, `E:\Projects\WorkDataHub\docs\business-background\年金计划类型与客户名称业务背景.md`, `docs/wiki-bi/_meta/legacy-semantic-map/`, current `wiki-bi` evidence pages | semantic-first canonical layer under `docs/wiki-bi/_meta/legacy-semantic-map/semantic/` plus compiler/reporting support that keeps execution-first discovery while aligning canonical outputs to real business semantics and durable `wiki-bi` targets | `docs` + `governance` + `platform` | `docs/superpowers/plans/2026-04-17-workdatahubpro-semantic-map-semantic-first-canonical-optimization.md` on branch `slice/semantic-map-semantic-first-canonical-pilot` from `slice/semantic-map-integration@002f4f7` | `planned` | semantic-schema/compiler/reporting tests, `customer-status-semantic-pilot` integration coverage, semantic question coverage report, full `uv run pytest -v` | N/A | this slice is semantic-map support work, not a new first-wave runtime rebuild; it exists to improve durable wiki absorption fidelity by compiling business-semantic conclusions directly and rejecting stub-only primary semantic sources |
