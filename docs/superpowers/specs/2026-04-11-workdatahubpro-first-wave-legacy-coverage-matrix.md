...
| `annual_award` | next recommended multi-sheet slice | no accepted executable slice yet |
| `annual_loss` | identified first-wave event domain | no accepted executable slice yet |
...
| Row ID | Behavior / Asset | Category | Legacy Source | Rebuild Target | Target Boundary | Owning Spec / Plan | Status | Validation Evidence | Retirement Decision | Notes / Risks |
|------|-------------------|----------|---------------|----------------|-----------------|--------------------|--------|---------------------|--------------------|---------------|
| AA-001 | multi-sheet workbook merge intake | capability | `docs/domains/annual_award-capability-map.md`, legacy migration workflow | future `capabilities/source_intake/annual_award/` slice | `capabilities` | architecture blueprint + follow-on multi-sheet validation plan | `planned` | future intake and replay tests | N/A | default next executable slice candidate |
| AA-002 | canonical award event transformation | capability | `docs/domains/annual_award-capability-map.md` | future `capabilities/fact_processing/annual_award/` | `capabilities` | architecture blueprint + follow-on multi-sheet validation plan | `planned` | future transform tests and replay evidence | N/A | event-domain semantics not yet rebuilt |
| AA-003 | optional `company_id` resolution path for award rows | mechanism | annual award capability map, legacy enrichment behavior | shared `identity_resolution` contract plus award-specific wiring | `capabilities` | architecture blueprint + follow-on multi-sheet validation plan | `planned` | future identity-path tests | N/A | must not couple domain code directly to providers |
| AA-004 | conditional plan-code enrichment from customer contract history | mechanism | annual award capability map | award-domain enrichment step with explicit evidence | `capabilities` | follow-on multi-sheet validation plan | `planned` | future replay and enrichment evidence | N/A | architecturally high-risk because it joins across domain history |
| AA-005 | customer-master backfill signals from award events | mechanism | annual award capability map and legacy backfill behavior | explicit derivation/publication path | `capabilities` + `platform` | follow-on multi-sheet validation plan | `planned` | future derivation/publication tests | N/A | should not return as hidden side effect |
| AA-006 | award fact publication used by downstream status logic | projection | annual award capability map, downstream snapshot dependency | explicit publication and replay fixture coverage | `platform` + `reference` | follow-on multi-sheet validation plan | `planned` | future replay and projection evidence | N/A | downstream dependency already exists in current first slice as fixture only |
