...
def test_run_first_wave_pilot_rebuilds_checked_in_registry_deterministically(
    tmp_path: Path,
) -> None:
    tmp_repo_root = tmp_path / "repo"
    registry_root = _copy_dependency_tree(tmp_repo_root)
    initial_claim_paths = pilot_claim_paths(registry_root)
    assert len(initial_claim_paths) == 18
    ...
    assert first_coverage["wave_id"] == FIRST_WAVE_PILOT_WAVE_ID
    ...
    second_result = run_first_wave_pilot(
        registry_root,
        claim_paths=pilot_claim_paths(registry_root),
    )
    assert (
